...
export function renderCompanyLogo(data: AdTemplateData): string {
  ...
  if (companion === 'headline' && data.title?.trim()) {
    ...
    companionHtml = `<div style="font-size:${fs}px;font-weight:800;color:#111827;line-height:1.2;">${escapeHtml(data.title.slice(0, 60))}</div>`;
...
export function renderTitle(data: AdTemplateData): string {
  ...
  if (!content) return '';
  return `<div style="text-align:center;font-size:${fontSize}px;font-weight:800;color:#111827;padding:8px 24px 20px;line-height:1.2;">${content}</div>`;
}
...
export function renderImage(product: ProductItem, heightPx: number): string {
  if (product.imageDataUri) {
    return `<img src="${product.imageDataUri}" alt="${escapeHtml(product.name)}" style="width:100%;height:${heightPx}px;object-fit:contain;border-radius:12px;background:#f8fafc;" />`;
  }
  return `<div style="width:100%;height:${heightPx}px;background:#f3f4f6;border-radius:12px;display:flex;align-items:center;justify-content:center;color:#9ca3af;font-size:14px;">No image</div>`;
}
